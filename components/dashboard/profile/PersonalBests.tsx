...
function Trophy({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    ...
    >
      <div style={{ fontSize: '0.65rem', opacity: 0.6, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{label}</div>
      <div style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--chart-5)', marginTop: 2 }}>{value}</div>
      <div style={{ fontSize: '0.65rem', opacity: 0.5, marginTop: 2 }}>{sub}</div>
    </div>
  );
}

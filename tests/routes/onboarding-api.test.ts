...
function mockSsrClient(authId: string | null, publicUser: { id: string; github_handle: string } | null) {
  return {
    auth: {
      getUser: vi.fn(async () => ({
    ...
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
          single: vi.fn(async () => ({ data: publicUser, error: publicUser ? null : new Error('not found') })),
        })),
      })),
    })),
  };
...
describe('POST /api/onboarding', () => {
  ...
  it('returns 400 for invalid team value', async () => {
    vi.doMock('@/lib/supabase/server', () => ({
      createClient: vi.fn(async () =>
        mockSsrClient('auth-1', { id: 'user-1', github_handle: 'bob' }),
      ),
    }));
    const { POST } = await import('../../app/api/onboarding/route');
  ...
  it('sets team=codex', async () => {
    vi.doMock('@/lib/supabase/server', () => ({
      createClient: vi.fn(async () =>
        mockSsrClient('auth-2', { id: 'user-2', github_handle: 'alice' }),
      ),
    }));
    const { POST } = await import('../../app/api/onboarding/route');
  ...
  it('writes user_private when opt-in=true and email is non-empty', async () => {
    vi.doMock('@/lib/supabase/server', () => ({
      createClient: vi.fn(async () =>
        mockSsrClient('auth-3', { id: 'user-3', github_handle: 'carol' }),
      ),
    }));
  ...
  it('does NOT write user_private when opt-in=false', async () => {
    vi.doMock('@/lib/supabase/server', () => ({
      createClient: vi.fn(async () =>
        mockSsrClient('auth-4', { id: 'user-4', github_handle: 'dave' }),
      ),
    }));
    const svcModule = await import('@supabase/supabase-js');
    const upsertSpy = vi.fn(async () => ({ error: null }));
  ...
  it('does NOT write user_private when opt-in=true but email is empty', async () => {
    vi.doMock('@/lib/supabase/server', () => ({
      createClient: vi.fn(async () =>
        mockSsrClient('auth-5', { id: 'user-5', github_handle: 'eve' }),
      ),
    }));
    const svcModule = await import('@supabase/supabase-js');
    const upsertSpy = vi.fn(async () => ({ error: null }));
